# Ubuntu 文件系统层次结构（Ubuntu Filesystem Hierarchy，`man hier`）

> 来源文件：`ubuntu_filesystem_hierarchy.txt`（2017 年前后整理的安装笔记）

原文为手册页 `man hier` 的纯文本提取结果。下方目录树与逐条目录说明均按原文出现的顺序排列，路径写法保持原样。
...
- **`/usr/share/sgml/mathtml`** — MathML DTD (optional).
